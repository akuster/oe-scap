...
xccdf = lxml.etree.parse("openembedded-xccdf.xml")
...
target_group = xccdf.find("//*[@id='xccdf_openembedded_group_redhat']")
...
for group in ssg.findall('{http://checklists.nist.gov/xccdf/1.2}Group'):
	...
	# Copy the whole group, now pruned, into the Openembeede document.
	target_group.append(group)

# Write out.
xccdf.write("openembedded-xccdf.xml", pretty_print=True)
